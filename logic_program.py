...
class LogicProgram(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super(LogicProgram, self).__init__()
        self.setupUi(self)

        # Conectar eventos de botones
        self.pushButton_5.clicked.connect(self.minimize_window)
        self.pushButton_6.clicked.connect(self.maximize_window)
        self.pushButton_2.clicked.connect(self.close_window)
        self.button_cam.clicked.connect(self.start_hand_detection_thread)# Conectar a la función que inicia la detección de manos
    ...
    def start_hand_detection_thread(self):
        # Iniciar la detección de manos en un hilo separado para no bloquear la interfaz de usuario
        hand_detection_thread = threading.Thread(target=self.start_hand_detection)
